...
# Terrain generation
def generate_terrain(size=500):
    terrain = np.random.randint(0, 100, (size, size))
    return terrain
...
def simulate_uav_2d(maze, terrain, path, speed_mph):
    fig, ax = plt.subplots()
    ...
    print(f"The highest point in the terrain is {highest_point} meters.")

def main():
    maze_size = 500
    maze = generate_maze(maze_size)
    terrain = generate_terrain(maze_size)
    start = (1, 1)
    ...
    if path:
        ...
        speed_mph = 45
        speed_mps = speed_mph * 1609.34 / 3600  # Convert speed to meters per second
        grid_size_meters = 2000 / maze_size  # Each grid cell represents a portion of the 2km^2 area
        ...
        simulate_uav_2d(maze, terrain, path, speed_mph)
    else:
        print("No path found.")
